    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                availableProducts.forEach(function (value:any) {
                    allProducts.push({ label: value.name, value: value.id });
});
                // Store available products in state
    ...
    const statusBodyTemplate = (rowData: any) => {
        const onStatusChange = async (newStatus: any) => {
            try {
                ...
                setOrders((prevOrders) => prevOrders.map((order) => (order.id === rowData.id ? { ...order, status: newStatus } : order)));
                toast.current?.show({
                    severity: 'success',
                    summary: 'تم التحديث',
        ...
        return (
            <div className="p-grid p-nogutter">
                <div className="p-col-6">
                    <Calendar value={options.value?.startDate || null} onChange={(e) => onDateChange(e.value, 'startDate', options)} placeholder="Start Date" dateFormat="yy-mm-dd" className="p-column-filter" aria-label="Start Date Filter" />
                </div>
                <div className="p-col-6">
                    <Calendar value={options.value?.endDate || null} onChange={(e) => onDateChange(e.value, 'endDate', options)} placeholder="End Date" dateFormat="yy-mm-dd" className="p-column-filter" aria-label="End Date Filter" />
                </div>
                    ...
                    <Dialog visible={orderDialog} style={{ width: '450px' }} header="تفاصيل الطلب" modal className="p-fluid rtl-text" footer={orderDialogFooter} onHide={hideDialog}>
                        ...
                        {orderItems.map((item, index) => (
                            <div key={index} className="grid field">
                                <div className="col-6">
                                    <p>حدد المنتج</p>
                                    <Dropdown value={item.product_id} options={products.map((product) => ({ label: product.name, value: product.id }))} onChange={(e) => updateOrderItem(index, 'product_id', e.value)} placeholder="حدد المنتج" />
                                </div>
                                <div className="col-3">
                                    <p>كمية</p>
                                    <InputNumber value={item.quantity} onValueChange={(e) => updateOrderItem(index, 'quantity', e.value)} min={1} placeholder="كمية" />
                                </div>
                                <div className="col-3">
                                    <p>السعر</p>
                                    <InputNumber
